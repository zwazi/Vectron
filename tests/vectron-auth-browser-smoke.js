...
// Firebase account, exercises account creation/login/logout/persistence, then
// deletes the account even if an assertion fails.
...
async function deleteTestAccount() {
    ...
    if(!signIn.ok) {
        ...
        if(response.error && ["EMAIL_NOT_FOUND", "INVALID_LOGIN_CREDENTIALS"].includes(response.error.message)) return;
        throw new Error("Could not retrieve disposable account for cleanup.");
    }
    const account = await signIn.json();
...
ws.onopen = async () => {
    try {
        ...
        console.log("Vectron Firebase account/login/logout browser smoke test passed.");
    } catch(error) {
        ...
        try {
            ...
            console.log("Disposable Firebase browser-test account deleted (if it was created).");
        } catch(cleanupError) {
